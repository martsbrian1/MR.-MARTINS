...
?>
	...
	<?php
		...
		if(empty($errorMessage)) {					
			$key = "SELECT trans_key FROM supplier_account WHERE trans_key = '".mysqli_real_escape_string($conn, $transkey)."' LIMIT 1";
			$fed = mysqli_query($conn, $key);
			
			if ($fed && mysqli_num_rows($fed) > 0 ) {			  
				$errorMessage = "<li><label style='color:green'><strong>Details saved successfully...</strong> <a href='./view_suppliers.php'>BACK</a></label></li> ";
			}else{
				$ref ="CR". serialNumber();
				$supplier_id= mysqli_real_escape_string($conn, $_GET['pid']);
				$trans_date  = $_POST['trans_date'];
				$goods_sup  = $_POST['goods_sup'];
				$amount_paid= $_POST['amount_paid'];
				$pay_type   = $_POST['pay_type'];
				$transkey  	= mysqli_real_escape_string($conn, $_POST['transkey']);

				$sql = "INSERT INTO supplier_account(supplier_id, trans_date, total_debit, total_credit, pay_type, staff, trans_key) 
						VALUES ('$supplier_id', '$trans_date', '$goods_sup', '$amount_paid', '$pay_type', '$sysuser', '$transkey')";
				
				/* $stmt = mysqli_prepare($conn, "SELECT current_balance FROM customer_accounts WHERE supplied_id = ?" );
						mysqli_stmt_bind_param($stmt, "i", $supplied_id);
						mysqli_stmt_execute($stmt); // supplier_transactions
...
<?php include"../inc/lay_sidebar.php";?>
  ...
  <div class="dew" style = "margin-left:10px"> <span style="color:blue; font-size:24px"><a href="#" title="Home"><?php echo @$title_main;?></a>  | <a href ="view_suppliers.php"> View Suppliers</a></span> </div>
  ...
  <tr>
    <td colspan="5" align="center" style="font-weight:bold; color:brown;"><h2><marquee direction="left" behavior="alternate"><?php //echo $loader . "'s  Payment" ?></marquee></h2></td>
  </tr>
  ...
  <tr>
	...
	<?php
		$qry=mysqli_query($conn,"SELECT * FROM suppliers WHERE `supplier_id`='".$supplier_id."'");
		$dew = mysqli_fetch_assoc($qry);
	print $name = $dew['supplier_name'] . " Making Payment";
		//$reg_no = $dew['reg_no'];
		$id    = $dew['supplier_id'];
  ...
  <tr>
    <td width="23%" style="font-size:14px"><strong>Payment Type:</strong></td>
    <td width="23%">
	<select name="pay_type" style="width:125px !important;box-shadow:2px 2px 2px 2px #555;padding:5px;border-radius:8px;border:1px solid red;">
      <option value="">--Select one--</option>
      <?php
		  $qry = mysqli_query($conn, "SELECT * FROM  pay_method");
		   while($ds = mysqli_fetch_assoc($qry)){
		   echo "<option value= ".$ds['id'].">".$ds['pay_method']."</option>";
		  }
		  ?>
    </select>
	</td>
  ...
  <tr>
	...
	<td><strong>Goods Supplied GHC: </strong></td>
    <td><input name="goods_sup" type="text" value="0.00" style="text-align:right; margin:5px 0;width:125px; !important;box-shadow:2px 2px 2px 2px #555;padding:5px;border-radius:10px;border:1px solid red; font-weight:bold" /></td>
    <td>&nbsp;</td>
  </tr>
  
  
  <tr>
    <td><!--<strong>Payment Type:XX</strong><em>  </em></b>--></td>
