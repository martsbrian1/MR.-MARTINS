<?php include"../inc/lay_header.php";
$msg = "";
	
	if (isset($_SESSION['username']))
	{
	$sysuser = $_SESSION['username'];
	$status = $_SESSION['status'];
	$store_id = $_SESSION['store_id'];
	$msg .="<li> Welcome  $sysuser!.</li>";

	}else{
	header('location:../index.php');
	}
?>	
  <!-- Header -->  
  
  <header>
  <div class="d-flex align-items-center">    
    <h2 class="mb-0 text-white" style="text-align:center; line-height:8px"><strong><?php echo @$company . " - " . @$store; ?></strong></h2>
  </div>
  <div style="text-align:right; font-size:13px; color:#fff; margin-top:-38px"><?php echo date("l jS \of F Y");?>
  <?php echo ucwords("$msg"); ?>
  </div>
</header>


  <!-- Sidebar -->  
<?php include"../inc/lay_sidebar.php";?>

  <!-- Main content -->
  <div class="content">
  <div class = "new_products" >
  <div class = "row" style = "padding-left:120px" >
  <span style="float:right; background-color:yellow; padding:4px; border:solid red; margin-right:40px;"><a href="cart.php">View Cart</a></span>
  <div class="dew" style = "margin-left: -5px" > <span class="tit"><a href="#" title="Home"><?php echo @$title_main;?></a></span><!--<span style="float:right; background-color:yellow; padding:4px; border:solid red"><a href="cart.php">View Cart</a>--></sp an></div>
    <fieldsety style="border:2px solid green; border-radius:8px;">
       <?php include"./prod_list_home.php";?>
    </fieldset>
  </div>

  <!-- Footer -->
  <?php include"../inc/lay_footer.php";?>
